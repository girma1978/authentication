...
const login = async (userInfo: UserLogin) => {
  try {
    ...
    const response = await fetch('/auth/login', {
      ...
      body: JSON.stringify(userInfo), // Passing the userInfo object in the request body
    });

    // Check if response is OK (status in range 200-299), else throw error
    if (!response.ok) {
      const errorData = await response.json(); // Parse error response as JSON
    ...
    return Promise.reject('Could not authenticate user. Please try again.');
  }
}
